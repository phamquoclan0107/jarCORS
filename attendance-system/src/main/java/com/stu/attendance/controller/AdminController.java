...
public class AdminController {
    ...
    public ResponseEntity<?> createSession(@Valid @RequestBody SessionDto sessionDto) {
        try {
            SessionDto createdSession = adminService.createSession(sessionDto);
            ...
            // Return specific error message for not found entities
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
            errorResponse.put("message", e.getMessage());
            return ResponseEntity.badRequest().body(errorResponse);
    ...
    @PutMapping("/sessions/{sessionId}")
    public ResponseEntity<?> updateSession(@PathVariable Integer sessionId, @Valid @RequestBody SessionDto sessionDto) {
        try {
            SessionDto updatedSession = adminService.updateSession(sessionId, sessionDto);
...
//   }
    @PostMapping("/student/add")
    public ResponseEntity<Map<String, Object>> addStudent(@RequestBody NguoiThamGiaDTO request){
        Map<String, Object> result = nguoiThamGiaService.addStudentWithResponse(request.getUserId(), request.getMaThamGia());
