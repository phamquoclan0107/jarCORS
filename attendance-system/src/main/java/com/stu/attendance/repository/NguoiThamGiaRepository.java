...
public interface NguoiThamGiaRepository extends JpaRepository<NguoiThamGia, Long> {
    ...
    // Thêm method kiểm tra sự tồn tại
    @Query("SELECT COUNT(ntg) > 0 FROM NguoiThamGia ntg WHERE ntg.nguoiDung.maNguoiDung = :maNguoiDung AND ntg.buoiHoc.maBuoiHoc = :maBuoiHoc")
    boolean existsByNguoiDung_MaNguoiDungAndBuoiHoc_MaBuoiHoc(@Param("maNguoiDung") String maNguoiDung, @Param("maBuoiHoc") Integer maBuoiHoc);
}
