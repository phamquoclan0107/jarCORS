...
import com.stu.attendance.dto.ZaloLoginRequest;
...
public class AuthService {
    ...
    /**
     * Authenticate user with Zalo and generate tokens
     */
    public LoginResponse authenticateWithZalo(ZaloLoginRequest zaloLoginRequest) {
        // Verify Zalo token and get user info
        ZaloService.ZaloUserInfo zaloUserInfo = zaloService.verifyZaloToken(zaloLoginRequest.getZaloAccessToken());
        
        // Check if Zalo ID matches
        if (!zaloUserInfo.getZaloId().equals(zaloLoginRequest.getZaloId())) {
            throw new RuntimeException("Zalo ID không khớp");
        }

        // Find account by Zalo ID
        TaiKhoan taiKhoan = authRepository.findByZaloId(zaloUserInfo.getZaloId())
                .orElseThrow(() -> new UsernameNotFoundException("Tài khoản Zalo không tồn tại trong hệ thống"));

        NguoiDung nguoiDung = authRepository.findNguoiDungByTaiKhoan(taiKhoan)
                .orElseThrow(() -> new RuntimeException("Thông tin người dùng không tồn tại"));

        // Create authentication token for Zalo user
        ...
        return new LoginResponse(accessToken, refreshToken, userInfoDto, taiKhoan.getRole(), false);
    }
